...
class OnlineRefer_Aduio(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, criterion, num_classes, num_queries, num_feature_levels,
        ...
        # self.tokenizer = RobertaTokenizerFast.from_pretrained('/home/user/OnlineRefer/data/Datasets/roberta-base/',
        #                                                       local_files_only=True)
        # self.text_encoder = RobertaModel.from_pretrained('/home/user/OnlineRefer/data/Datasets/roberta-base/',
        #                                                  local_files_only=True)

        ###########-------Audio Encoder--------##############
        self.whisper_model = whisper.load_model("small")
        self.audio_encoder = self.whisper_model.encoder
    ...
    def forward_single_frame(self, samples: NestedTensor, track_res, captions, targets,audios):
        ...
        audio_features = self.forward_audio(audios,device=pos[0].device)
        ...
        # Segmentation
        mask_features = self.pixel_decoder(features, audio_features, pos, memory,
    ...
    def inference(self, samples:NestedTensor, track_res, captions, targets,audios):

        if samples is not NestedTensor:
            frame = nested_tensor_from_videos_list(samples)

        frame_res = self.forward_single_frame(frame, track_res, captions, targets,audios)

        return frame_res

    def forward(self, samples:NestedTensor, captions, targets,audios):
        ...
        for frame_index in range(num_frames):
            ...
            target_i = {k: targets[0][k][frame_index].unsqueeze(0) for k in targets[0].keys() if (k != 'orig_size' and k !='size' and k != 'yolov7_det' and k != 'audio')}
            target_i['orig_size'] = targets[0]['orig_size']
            target_i['size'] = targets[0]['size']
            target_i['audio'] = targets[0]['audio']
            target_i = [target_i]
            ...
            if self.use_checkpoint_for_more_frames and frame_index < num_frames - 1:
                def fn(frame, *args):
                    ...
                    frame_res = self.forward_single_frame(frame, tmp, captions, target_i,audios)
                    return (
                ...
                frame = nested_tensor_from_videos_list(frame)
                frame_res = self.forward_single_frame(frame, track_res, captions, target_i,audios)
    ...
    def forward_audio(self,audios,device):
        encoded = []
        # print('#################This is audios#################')
        # print(audios,len(audios))
        for audio in audios:
            encoded.append(self.audio_encoder(audio.to(device)))
        encoded = torch.cat(encoded, dim=0).to(device)
        audio_features = self.resizer(encoded)
        audio_features = self.avgpool(audio_features)
...
class AvgPool(nn.Module):
    def __init__(self):
        ...
        self.avg_pool = nn.AdaptiveAvgPool1d(output_size=12)  # 根据输入和输出维度调整kernel_size和stride
...
def build(args):
    ...
    #################
    print('#############################################################')
    print('This is Model Information')
    print('#############################################################')
    model_module = set()
    checkpoint_module = set()
    for name in model.state_dict():
        model_module.add(name)

    # print('#############################################################')
    # print('Checkpoint Information')
    # print('#############################################################')
    checkpoint = torch.load('/home/user/OnlineRefer_Modified/ytvos-r50-checkpoint.pth')
    checkpoint_dict = {}
    for name, param in checkpoint['model'].items():
        checkpoint_dict[name] = param
        checkpoint_module.add(name)

    print(len(model_module),len(checkpoint_module),len(model_module&checkpoint_module))

    # for name in model.state_dict():
    #     if name in checkpoint_module:
    #         print("{}parameters has been loaded".format(name))
    #         model.get_parameter(name).data = checkpoint_dict[name]
